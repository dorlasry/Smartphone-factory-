...
public abstract class Task<R> {
    ...
    protected final void whenResolved(Collection<? extends Task<?>> tasks, Runnable callback) {
    	if(tasks.size()==0)
    		...
    		AtomicInteger num=new AtomicInteger(tasks.size());						//needs AtomicInteger in case two tasks end simultaneously
    		for(Iterator<? extends Task<?>> i=tasks.iterator(); i.hasNext();){		//runs the callback once all the sub-tasks were completed
    			i.next().getResult().whenResolved(()->{
